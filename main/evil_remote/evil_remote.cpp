...
int Myregion=1; // hardcode north america for now
bool sendIR=false;
...
EvilRemote::EvilRemote(std::string name, TFT_eSprite* sprite) : ArduinoRoutine(name, sprite) {}

void EvilRemote::setup() {
...
void EvilRemote::handle_event(int event) {
  if (event == EVENT_SEND_CODES) {
    send_all_codes();
  }
}

void EvilRemote::send_all_codes() {
  digitalWrite(LED, LOW);
  if (this->sprite) {
    this->sprite->drawString("Transmitting...", 2, sprite->height() / 2);
    this->sprite->pushSprite(0, 0); // TODO
  }
